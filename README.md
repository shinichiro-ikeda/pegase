...
As an example, we will define a language capable of performing the four basic arithmetic operations and save it as `basic_calc.peg`.
...
We parse this grammar definition file (`basic_calc.peg`) using the `parse` method of the previously created PEG grammar object, and then evaluate the resulting abstract syntax tree (AST) to obtain the syntax rules (R) of `basic_calc.peg`.
...
Create a Python package named `basic_calc_VN.py` containing non-terminal symbol objects for a language that supports the four basic arithmetic operations.
...
Create the main program for the four basic arithmetic operations as `basic_calc.py`.
